...
const Banner = ({ data, handleOrderPopup }) => {
  ...
  useEffect(() => {
    const updateCountdown = () => {
      const now = dayjs();
      const difference = countdownEnd.diff(now);
      ...
      <div className='container'>
        <div style={{ backgroundColor: data.bgColor }} className='grid grid-cols-1 md:grid-cols-3 gap-6 items-center text-white rounded-3xl'>
          <div className='p-6 sm:p-8'>
            ...
            <p data-aos='fade-up' className='text-lg font-semibold'>Ends in: {`${timeLeft.hours}h ${timeLeft.minutes}m ${timeLeft.seconds}s`}</p>
          </div>
